...
(function(){
   ...
   function uuidv4(){
    ...
    function messageListener(e){
        let self = this.self;
        self.dispatch("message", e.data, this.sender_id, self.id);
    }
    ...
    class P2PRoom
    {
        constructor({
            ...
            for(let peer_id of peer_list){
                if(peer_id == this.id){
                    this.peer_list.push(this.id);
                } else{
                    this.add(peer_id);
                }
            }
        ...
        add(id){
            if(this.peer_list.contains(id)){
                __console.warn("webrtc.js: adding repeated pear:", id);
                return;
            }
            let pc = this.local_peer.connect(id);
            this.peer_list.push(id);
            pc.on("message", messageListener.bind({
                    self: this,
                    sender_id: id,
            }));
            this.id_to_conn[id] = pc;
        }
        remove(id){
            let index = this.peer_list.indexOf(id);
            if(index >= 0){
                this.peer_list.splice(index, 1);
                let pc = this.id_to_conn[id];
                pc.close();
                delete this.id_to_conn[id];
            } else{
                console.warn("P2PRoom: removing non-existed peer connection");
            }
        }


        sendTo(id, data){
            this.id_to_conn[id].send(data);
        }
        sendToAll(data){
            for(let pc of this.id_to_conn){
                pc.send(data);
            }
        }


        on(type, listener, config = { once: false, }){
            switch(type){
            case "message":
                this.listeners[type].push([listener, config]);
                break;
